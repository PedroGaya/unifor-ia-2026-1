...
beta_hat = np.linalg.inv(X_ones.T @ X_ones) @ X_ones.T @ y

x_line = np.linspace(X_ones.min(), X_ones.max(), 200)
...
def validate(X, y, rounds=500, test_size=0.2):
    n = len(y)
